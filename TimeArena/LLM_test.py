...
import prompt
from Agent import *
from src.taes.timearena import TAES_Agent
...
def Run(args):
    ...
    elif args.prompting == 'taes':
        Agent = TAES_Agent(B=3, K=3, plan_depth=20)
        Agent.initialize_state(env, total_time)
    else:
        Agent = Raw_Agent()
...
def parse():
    ...
    parser.add_argument("--prompting", type=str, default=None, choices=['react','reflexion','selfplan','taes'], help='The prompting method')
    ...
    parser.add_argument("--lm", type=str, choices=['gpt4','gpt3.5','openchat','mistral','vicuna','mixtral-moe','gemini','custom'],help='Model to evaluate')
