...
class TimeArena():
    ...
    def constraint_only_for_task(self, objects_in_task, name2objects):
        self.constraint_only_for_task_dict = {}
        shared_resource_names = ("pot", "fryer", "oven")

        for task_objects in objects_in_task.values():
            for subject in task_objects:
                subject_name = subject.properties['name']
                for action in subject.properties['todo']:
                    if "_1" not in action:
                        continue
                    corresponding_action = action.replace("_1", "_2")
                    for container in task_objects:
                        container_name = container.properties['name']
                        if corresponding_action not in container.properties['todo']:
                            continue
                        if any(resource in container_name for resource in shared_resource_names):
                            continue
                        allowed_subjects = self.constraint_only_for_task_dict.setdefault(container_name, [])
                        if subject_name not in allowed_subjects:
                            allowed_subjects.append(subject_name)
