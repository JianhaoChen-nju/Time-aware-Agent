...
if __name__ == '__main__':
    ...
    output_file = args.output_file or f'{args.tmp_dir}/{args.set_type}_{args.model_name}{suffix}_{args.mode}.txt'

    total_price = 0
    end_idx = args.end_idx or len(data)
    selected = list(enumerate(data))[args.start_idx - 1:end_idx]
    for idx, prompt in tqdm(selected):
        if prompt == "":
