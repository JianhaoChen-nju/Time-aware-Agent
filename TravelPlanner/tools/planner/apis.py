...
class Planner:
    def __init__(self,
        ...
        if model_name in  ['mistral-7B-32K']:
            ...
            self.llm = ChatOpenAI(model_name=LLM_MODEL_NAME, temperature=0, max_tokens=4096,
                                  openai_api_key=OPENAI_API_KEY, openai_api_base=LLM_BASE_URL)
        else:
...
class ReactPlanner:
    ...
    def __init__(self,
        ...
        if model_name == 'custom':
            self.react_llm = ChatOpenAI(model_name=LLM_MODEL_NAME, temperature=0, max_tokens=1024,
                                        openai_api_key=OPENAI_API_KEY, openai_api_base=LLM_BASE_URL,
    ...
    def __init__(self,
        ...
        if model_name in ['gemini']:
            ...
            self.react_llm = ChatOpenAI(model_name=LLM_MODEL_NAME, temperature=0, max_tokens=1024,
                                        openai_api_key=OPENAI_API_KEY, openai_api_base=LLM_BASE_URL,
                                        model_kwargs={"stop": ["Action","Thought","Observation,'\n"]})
            self.reflect_llm = ChatOpenAI(model_name=LLM_MODEL_NAME, temperature=0, max_tokens=1024,
                                          openai_api_key=OPENAI_API_KEY, openai_api_base=LLM_BASE_URL,
                                          model_kwargs={"stop": ["Action","Thought","Observation,'\n"]})
        else:
            self.react_llm = ChatOpenAI(model_name=model_name, temperature=0, max_tokens=1024, openai_api_key=OPENAI_API_KEY,model_kwargs={"stop": ["Action","Thought","Observation,'\n"]})
